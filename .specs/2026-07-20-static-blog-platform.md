...
Cross-cutting note: the automated test suite in Test Strategy is **not yet
written**. Requirements below were verified manually against a real
`npm run build` (10 pages, sitemap/robots/RSS/Pagefind index emitted, SEO tags
and responsive WebP covers present in `dist`). Writing those tests is the next
implementation task.
