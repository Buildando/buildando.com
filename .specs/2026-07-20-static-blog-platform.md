...
- `REQ-020`: Done. Behind the `src/integrations/Search.astro` port; the Pagefind adapter (`search/pagefind/Pagefind.astro`) renders the UI into a given mount and self-inits when visible. `dist/pagefind/` present.
...
- `REQ-036`: Done. `src/components/SearchModal.astro` is now a provider-agnostic shell (native `<dialog>`, `/` and Ctrl/Cmd+K) hosting the `Search` port and triggering its init on open; mounted once in `src/layouts/BaseLayout.astro`; header trigger in `src/components/Header.astro` degrades to a `/search` link. Verified in `dist`: trigger + dialog on every page. `/search` still covers `REQ-020`.
