...
- `REQ-035`: The home page shall let the reader filter the displayed post list interactively by facets — at least category and publication date, and extensibly by tag — on the client, without a full page navigation, and with no backend. The filtering shall be additive to the existing per-facet listing pages (`REQ-019`), not a replacement, and shall degrade to the full list when JavaScript is unavailable. The exact interaction model (chips, dropdowns, a date range, URL-encoded filter state) is an open design question, tracked below and not yet implemented.
- `REQ-036`: Search shall be reachable from the site layout itself — not only from a dedicated `/search` page — so a reader can start a search from any page. The presentation (a header field, a launched modal/overlay, an inline box) is an open design question, tracked below and not yet implemented; it shall reuse the client-side, backend-free index of `REQ-020`.
...
- **On-home faceted filtering (`REQ-035`) is specified but undesigned.** The list to solve: which facets (category and date are required; tags likely; author possibly), the control shape (chips vs dropdowns vs a sidebar), whether date is a range or buckets (year/month), and whether filter state lives in the URL so a filtered view is shareable and back-button-friendly. It must stay client-only and must not fight the static per-facet pages that already exist. A likely implementation is a small progressive-enhancement island over the already-rendered cards, hidden entirely when JS is off. Decide before building.
- **In-layout search (`REQ-036`) is specified but undesigned.** "I don't yet know how it will work" — the open choices are placement (a header search field, a keyboard-shortcut modal, an inline box on the home) and whether it replaces or complements the `/search` page. Whatever the shape, it reuses the Pagefind index, so the decision is UX, not architecture. The current `/search` page satisfies `REQ-020` in the meantime.
...
- `test/build.test.ts` — assertions on `dist/` (root redirect, per-locale
  ...
  fallback page, the Pagefind index) plus a source scan enforcing `REQ-030`.
...
- `REQ-035`: Pending. On-home faceted filtering (category, date, tags). Design open — see Risks.
- `REQ-036`: Pending. Search reachable from the layout. Design open — see Risks. `/search` covers `REQ-020` meanwhile.
