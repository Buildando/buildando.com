...
- `REQ-043`: Done. `INTEGRATIONS` in `src/config/site.ts` selects the comments provider; `src/integrations/Comments.astro` is the port, with `comments/giscus/` and `comments/utterances/` adapters and a contributor guide in `src/integrations/README.md`. Verified: default (giscus, unconfigured) renders nothing; configuring giscus renders it; switching `comments` to `"utterances"` (+ its repo) swaps the widget with no other change, and the unused adapter ships nothing.
...
- `REQ-042`: Done. `CONSENT` in `src/config/site.ts`; `src/components/ConsentBanner.astro` (mounted in `BaseLayout`) holds the GA4/AdSense loaders and injects them only after "Accept" (or immediately when `required: false`), remembering the choice in `localStorage`; Plausible stays in `BaseHead` (cookieless). Nothing renders when neither GA nor AdSense is set. Verified: default build has no banner/scripts; with GA+AdSense configured, the banner and Accept/Reject appear and no `gtag`/`adsbygoogle` `<script src>` loads before consent.
