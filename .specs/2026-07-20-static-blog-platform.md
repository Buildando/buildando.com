...
- `REQ-041`: The site shall provide a date archive: an index page grouping published posts by year and month — with per-month counts and localized month names — and a page per month listing that month's posts. It shall be reachable from the navigation, be per-locale, and require no backend.
...
- `REQ-041`: Done. `getArchive` in `src/lib/posts.ts` groups posts by UTC year/month; `src/pages/[lang]/archive/index.astro` renders the year→month tree with counts and localized month names, and `src/pages/[lang]/archive/[year]/[month].astro` lists a month's posts with a localized "Month Year" heading; a `nav.archive` entry links it. Verified in `dist`: per-locale index and month pages, `Julho`/`July`, "Julho de 2026", and the counts.
