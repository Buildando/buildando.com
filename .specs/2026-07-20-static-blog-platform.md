...
- `REQ-040`: Done. `SHARE` in `src/config/site.ts`; `src/components/ShareButtons.astro` renders share-intent links (no JS) plus a clipboard "copy link" revealed only when supported, mounted at the end of `src/layouts/PostLayout.astro`. Verified: post pages carry the intent links; the row hides when `SHARE` is emptied.
